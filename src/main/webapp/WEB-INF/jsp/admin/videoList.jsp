...
<script type="text/javascript">
	...
	function doDelete(){
		...
		$.messager.confirm('提示','你确定要删除吗?',function(r){
			if(r){
			       ...
			       //ajax提交数组参数，必须序列号
			       $.post("${pageContext.request.contextPath}/admin/video/delete",{"ids":ids},function(data){
			       		if(data=="200"){
				       		location.href="${pageContext.request.contextPath}/admin/video/list";
	...
	var columns = [ [ {
		...
		formatter:function(value, row, index){
			/* if(row.url){ */
				return "<img style='width:36px;height:36px;border-radius:50%' border='1' src='${pageContext.request.contextPath}/upload/"+row.imageUrl+"'/>";
			//}
		}
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			//pagination : true,
			toolbar : toolbar,
			url : "${pageContext.request.contextPath}/admin/video/list/json",
			idField : 'id',
			columns : columns

    ...
    <div class="row">
        <div class="col-md-2 text-right" style="margin-top:10px;">
            <ul class="nav  nav-tabs flex-column">
                <li class="nav-item ">
                    <a class="nav-link  active" href="${pageContext.request.contextPath }/job/all" style=" width:100px;"
                       style=" background-color:#000">批改作业</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="${pageContext.request.contextPath }/job/teacher/show_fabu"
                       style=" width:100px;">发布作业</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="${pageContext.request.contextPath }/job/teacher/show_tijiao"
                       style=" width:100px;">提交作业</a>
                </li>
            </ul>
        </div>
                    ...
                    <select name="banjiId" class="form-control" id="banjiId">
                        ...
                        <c:forEach items="${banjis}" var="banji">
                            <option value="${banji.id }"
                                    <c:if test="${banji.id==banjiId }">selected</c:if>>${banji.name }</option>
                        </c:forEach>
                    </select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            ...
            <table class="table table-hover" style="font-size:0.9rem;">
                ...
                <c:forEach items="${lists}" var="tp">
                    <tr>
                        ...
                        <td><a href="${pageContext.request.contextPath }/download/job?filename=${tp.stu_job_url}">下载</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
...
<%--  <div class="container">
   <ul class="pagination justify-content-end">
      <c:if test="${pageInfo.size>0 }">
          <li class="util-item"><a class="util-link" href="${pageContext.request.contextPath }/job/all?pageNum=1&name=${name}&banjiId=${banjiId}&professionId=${professionId}">首页</a></li>
      </c:if>
      <c:if test="${pageInfo.hasPreviousPage }">
          <li class="util-item"><a class="util-link" href="${pageContext.request.contextPath }/job/all?pageNum=${pageInfo.pageNum-1}&name=${name}&banjiId=${banjiId}&professionId=${professionId}">&laquo;</a></li>
      </c:if>

       <c:forEach items="${pageInfo.navigatepageNums}" var="page_num">
           <c:if test="${pageInfo.pageNum ==page_num }">
               <li class="util-item active"><a class="util-link" href="${pageContext.request.contextPath }/job/all?pageNum=${page_num }&name=${name}&banjiId=${banjiId}&professionId=${professionId}">${page_num }</a></li>
           </c:if>
           <c:if test="${pageInfo.pageNum !=page_num }">
               <li class="util-item"><a class="util-link" href="${pageContext.request.contextPath }/job/all?pageNum=${page_num }&name=${name}&banjiId=${banjiId}&professionId=${professionId}">${page_num}</a></li>
           </c:if>
       </c:forEach>

    <c:if test="${pageInfo.hasNextPage }">
      <li class="util-item"><a class="util-link" href="${pageContext.request.contextPath }/job/all?pageNum=${pageInfo.pageNum+1}&name=${name}&banjiId=${banjiId}&professionId=${professionId}">&raquo;</a></li>
  </c:if>

  <c:if test="${pageInfo.size>0}">
          <li class="util-item"><a class="util-link" href="${pageContext.request.contextPath }/job/all?pageNum=${pageInfo.pages }&name=${name}&banjiId=${banjiId}&professionId=${professionId}">末页</a></li>
      </c:if>
  </ul>
 </div>  --%>
<!-- 页脚-->
